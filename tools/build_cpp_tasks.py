...
struct P { const std::string& z; size_t i=0; void ws(){while(i<z.size()&&isspace((unsigned char)z[i]))++i;} char take(){ws();return i<z.size()?z[i++]:0;} J val(){ char c=take(); if(c=='"'){J q;q.k=J::S;while(i<z.size()&&z[i]!='"'){if(z[i]=='\\')++i;q.s+=z[i++];}++i;return q;} if(c=='['){J q;q.k=J::A;ws();if(i<z.size()&&z[i]==']'){++i;return q;}--i;for(;;){q.a.push_back(val());ws();char d=z[i++];if(d==']')break;}return q;} if(c=='{'){J q;q.k=J::O;ws();if(i<z.size()&&z[i]=='}'){++i;return q;}--i;for(;;){J key=val();take();q.o[key.s]=val();ws();char d=z[i++];if(d=='}')break;}return q;} --i; size_t st=i;while(i<z.size()&&!strchr(",]} \t\r\n",z[i]))++i;std::string t=z.substr(st,i-st);J q;if(t=="null")q.k=J::N;else if(t=="true"||t=="false"){q.k=J::B;q.b=t=="true";}else{q.k=J::X;q.x=t;}return q;} };
...
def cpp_runner(task_id):
    ...
    runners = {
        ...
        "S4-A": "int main(){auto x=runner::vi(runner::read());auto y=runner::integer(runner::read());std::cout<<runner::out(solution(x,y));}",
    ...
    if task_id not in runners:
        ...
        if task_id == "S4-D":
            return "int main(){auto a=runner::read();std::vector<Value>v;for(auto&x:a.a){if(x.k==runner::J::S)v.push_back(x.s);else if(x.k==runner::J::B)v.push_back(x.b);else if(x.k==runner::J::N)v.push_back(std::monostate{});else if(runner::decimal(x))v.push_back(runner::real(x));else v.push_back(runner::integer(x));}std::cout<<runner::out(solution(v));}"
        if task_id == "S3-D":
